 ...
 * 分析コードの重複チェック
 */
export async function checkAnalysisCodeExists(
  analysisCode: string,
): Promise<{ exists: boolean; analysisCode?: Partial<AnalysisCode> }> {
  try {
 ...
 * 分析コードの削除
 */
export async function deleteAnalysisCode(
  analysisCode: string,
 ...
 * 分析コードの検索
 */
export async function searchAnalysisCodes(
  searchTerm: string,
  filters: {
    isActive?: boolean;
    analysisType?: string;
