...
import { PartnerMasterList } from "@/components/accounting/partner-master-list";
      ...
      <main className="container mx-auto px-8 py-8 space-y-8 max-w-7xl">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-card/40 backdrop-blur-sm rounded-xl p-6 shadow-md border">
          <div className="space-y-2">
            <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-foreground to-muted-foreground bg-clip-text text-transparent">取引先マスタ</h1>
        ...
        <Card className="shadow-xl border-0 bg-card/60 backdrop-blur-sm overflow-hidden">
          <CardHeader className="bg-gradient-to-r from-muted/20 to-muted/10 pb-8">
            <CardTitle className="text-2xl font-bold">取引先一覧</CardTitle>
          ...
          <CardContent className="p-8">
            <Suspense fallback={
              <div className="flex items-center justify-center py-16">
                ...
                <span className="ml-4 text-muted-foreground">読み込み中...</span>
              </div>
            }>
              <PartnerMasterList key={refreshTrigger} />
            </Suspense>
          </CardContent>
        </Card>

        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl border-0">
            <DialogHeader className="space-y-4 pb-6">
              <DialogTitle className="text-2xl font-bold">新規取引先作成</DialogTitle>
