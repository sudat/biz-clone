    ...
    switch (toolName) {
      ...
      case "search_journals":
        ...
        const journalResponse = await fetch(
          `${baseUrl}/api/journal?${journalParams}`,
        );
      ...
      case "search_accounts":
        ...
        if (args.accountType) {
          accountParams.set("accountType", args.accountType);
        }
        if (args.isActive !== undefined) {
          accountParams.set("isActive", args.isActive.toString());
        }

        const accountResponse = await fetch(
          `${baseUrl}/api/master/accounts?${accountParams}`,
        );
      ...
      case "search_partners":
        ...
        if (args.partnerType) {
          partnerParams.set("partnerType", args.partnerType);
        }
        if (args.isActive !== undefined) {
          partnerParams.set("isActive", args.isActive.toString());
        }

        const partnerResponse = await fetch(
          `${baseUrl}/api/master/partners?${partnerParams}`,
        );
      ...
      case "get_trial_balance":
        ...
        const trialResponse = await fetch(
          `${baseUrl}/api/reports/trial-balance?${trialParams}`,
        );
      ...
      case "unified_search":
        ...
        const searchResponse = await fetch(
          `${baseUrl}/api/search/unified?${searchParams}`,
        );
...
export async function GET() {
  return NextResponse.json({
    ...
    version: "1.0.0",
    description:
      "Biz Clone Accounting MCP Server for journal and master data management",
    capabilities: {
      tools: {},
    },
    endpoints: {
...
// PUT method removed to prevent Claude Code from attempting DCR
