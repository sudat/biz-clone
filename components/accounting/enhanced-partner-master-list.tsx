...
const sortOptions: SortOption[] = [
  { field: "partner_code", label: "コード順" },
  { field: "partner_name", label: "名称順" },
  { field: "partner_kana", label: "かな順" },
  { field: "partner_type", label: "種別順" },
  { field: "created_at", label: "作成日順" },
];

export function EnhancedPartnerMasterList() {
  const [partners, setPartners] = useState<Partner[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchState, setSearchState] = useState<SearchState>({
    searchTerm: "",
    filters: {},
    sortField: "partner_code",
    sortDirection: "asc",
    activeOnly: false,
  });
  ...
  const loadPartners = async () => {
    setLoading(true);
    try {
      const result = await PartnerDataAdapter.getPartners();
  ...
  const performServerSearch = useCallback(async (searchState: SearchState) => {
    setLoading(true);
    try {
      const result = await PartnerDataAdapter.searchPartners(
        searchState.searchTerm,
        {
          partner_type: searchState.filters.partner_type,
  ...
  const handleDelete = async (partner: Partner) => {
    if (confirm(`取引先「${partner.partner_name}」を削除しますか？`)) {
      try {
        const result = await PartnerDataAdapter.deletePartner(
            ...
            <TableBody>
              {filteredPartners.length === 0 ? (
                ...
                filteredPartners.map((partner) => (
                  <TableRow key={partner.partner_code}>
                    <TableCell className="font-mono">
                      {renderHighlightedText(partner.partner_code)}
                    </TableCell>
                    <TableCell className="font-medium">
                      {renderHighlightedText(partner.partner_name)}
                    </TableCell>
                    <TableCell>
                      {renderHighlightedText(partner.partner_kana)}
                    </TableCell>
                    <TableCell>
                      {renderHighlightedText(partner.partner_type)}
                    </TableCell>
