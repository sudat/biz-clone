...
function SingleConflictResolver<T>({
  ...
  const [resolutionMode, setResolutionMode] = useState<"quick" | "manual">(
    "quick"
  );
    ...
    for (const field of changedFields) {
      ...
      (merged as any)[field] =
        selectedSource === "local"
          ? (localData as any)[field]
          : (remoteData as any)[field];
    }
      ...
      <div className="flex items-start space-x-3 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-yellow-700 mt-1">
            他のユーザーと同時に編集を行ったため、{changedFields.length}
            個のフィールドで競合が発生しています。
          </p>
          {conflict.timestamp && (
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden">
        ...
        <DialogFooter className="flex items-center justify-between">
          <div className="text-xs text-muted-foreground">
            {conflicts.length > 1 &&
              `残り ${
                conflicts.length - currentConflictIndex - 1
              } 件の競合があります`}
          </div>
