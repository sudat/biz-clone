...
export const accountSchema = z.object({
  accountCode: z.string().max(10, "勘定科目コードは10文字以内で入力してください"),
...
export const departmentSchema = z.object({
  departmentCode: z.string().max(10, "部門コードは10文字以内で入力してください"),
...
export const analysisCodeSchema = z.object({
  ...
  analysisName: z.string().max(100, "分析コード名は100文字以内で入力してください"),
...
export class ApiException extends Error {
  ...
  constructor(type: ApiErrorType, message: string, statusCode?: number, details?: any, code?: string) {
    super(message);
  ...
  private getDefaultStatusCode(type: ApiErrorType): number {
    switch (type) {
