...
import { ApiException, ApiErrorType, JournalSaveInput, SearchOptions } from '@/lib/api/types';
 ...
 * マスタデータの存在チェック
 */
async function validateMasterData(details: JournalSaveInput['details']): Promise<void> {
  const accountCodes = [...new Set(details.map(d => d.accountCode))];
  const partnerCodes = [...new Set(details.map(d => d.partnerCode).filter(Boolean))] as string[];
  const departmentCodes = [...new Set(details.map(d => d.departmentCode).filter(Boolean))] as string[];
  const analysisCodes = [...new Set(details.map(d => d.analysisCode).filter(Boolean))] as string[];
  ...
  if (analysisCodes.length > 0) {
    ...
    if (missingAnalysisCodes.length > 0) {
      throw new ApiException(
        ApiErrorType.BUSINESS,
        `存在しない分析コードが含まれています: ${missingAnalysisCodes.join(', ')}`,
        422,
 ...
 * 新規仕訳保存
 */
export async function saveJournal(data: JournalSaveInput): Promise<JournalWithDetails> {
  // バリデーション
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    const journalNumberResult = await JournalNumberService.generateNextJournalNumber(
    ...
    // 合計金額計算
    const totalAmount = data.details.reduce((sum, detail) => sum + detail.totalAmount, 0) / 2;
    ...
    return {
      ...
      details: details.map(detail => ({
        ...detail,
        id: (detail as any).id?.toString() || detail.lineNumber?.toString() || '1',
 ...
 * 単一仕訳取得
 */
export async function getJournalByNumber(journalNumber: string): Promise<JournalWithDetails | null> {
 ...
 * 仕訳検索
 */
export async function searchJournals(params: JournalSearchParams): Promise<PaginatedJournalResult> {
  ...
  // 明細フィルター
  if (params.filters?.accountCode || params.filters?.partnerCode || 
      params.filters?.departmentCode || params.filters?.analysisCode) {
    where.journalDetails = {
      some: {
        ...(params.filters.accountCode && { accountCode: params.filters.accountCode }),
        ...(params.filters.partnerCode && { partnerCode: params.filters.partnerCode }),
        ...(params.filters.departmentCode && { departmentCode: params.filters.departmentCode }),
        ...(params.filters.analysisCode && { analysisCode: params.filters.analysisCode })
      }
    };
  }
